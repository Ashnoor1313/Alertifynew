...
export default function SmsPage() {
  ...
  const handlePredict = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${FASTAPI_URL}/sms/predict`, {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-emerald-900 to-green-900 flex items-center justify-center px-4">
      <div className="bg-black/40 backdrop-blur-lg border border-emerald-700 p-8 rounded-3xl shadow-2xl text-center max-w-md w-full">
        <h1 className="text-2xl font-bold text-emerald-400 mb-2">💬 SMS Spam Detection</h1>
        ...
        {result && (
          <div className="mt-8 bg-black/40 rounded-2xl p-5 text-left shadow-inner border border-emerald-700">
            ...
            <p className="text-gray-300 text-sm"><b>SMS:</b> {result.text}</p>
            <p className="text-gray-300 text-sm mt-2">
              <b>Prediction:</b>{" "}
              <span className={result.prediction === "Spam" ? "text-red-500 font-semibold" : "text-emerald-400 font-semibold"}>
                {result.prediction === "Spam" ? "Spam / Unsafe" : "Safe / Trusted"}
              </span>
            </p>
