...
export default function UpiPage() {
  ...
  const handlePredict = async () => {
    if (!upi.trim()) return alert("Please enter a UPI ID");
    if (!validateUpi(upi)) return alert("Invalid UPI ID entered. Please enter a correct UPI ID");
    try {
      setLoading(true);
      const response = await fetch(`${FASTAPI_URL}/upi/predict`, {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-emerald-900 to-green-900 flex items-center justify-center px-4">
      <div className="bg-black/40 backdrop-blur-lg border border-emerald-700 p-8 rounded-3xl shadow-2xl text-center max-w-md w-full">
        <h1 className="text-2xl font-bold text-emerald-400 mb-2">💸 UPI Fraud Detection</h1>
        ...
        {result && !result.error && (
          <div className="mt-8 bg-black/40 rounded-2xl p-5 text-left shadow-inner border border-emerald-700">
            ...
            <p className="text-gray-300 text-sm"><b>UPI:</b> {result.upi}</p>
            <p className="text-gray-300 text-sm mt-1">
              <b>Prediction:</b>{" "}
              <span className={result.prediction === 0 ? "text-emerald-400 font-semibold" : "text-red-500 font-semibold"}>
                {result.prediction === 0 ? "Safe / Genuine" : "Spam / Fraudulent"}
              </span>
            </p>
            {result.probability && (
              <p className="text-gray-300 text-sm mt-1">
                <b>Confidence:</b> {((result.probability[result.prediction] || 0) * 100).toFixed(2)}%
              </p>
            )}
          </div>
        )}
