...
export default function QrScanner() {
  ...
  const handleUpload = async () => {
    if (!file) return toast.error("Please select a QR image first!");
    ...
    try {
      const res = await axios.post(`${FASTAPI_URL}/qr/`, formData, {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-emerald-900 to-green-900 flex items-center justify-center px-4 relative">
      ...
      <div className="bg-black/40 backdrop-blur-lg text-white p-8 rounded-3xl shadow-2xl max-w-md w-full border border-emerald-600">
        <h2 className="text-2xl font-bold text-center text-emerald-400 mb-2">🔍 QR Code Spam Detection</h2>
        ...
        <div className="flex flex-col items-center gap-3">
          <label htmlFor="fileInput" className="cursor-pointer bg-emerald-700 hover:bg-emerald-800 text-white px-4 py-2 rounded-lg shadow-md transition">
            Choose QR Image
          </label>
          <input id="fileInput" type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
          {file && <p className="text-sm text-gray-400">Selected: <span className="text-white">{file.name}</span></p>}
          <button
        ...
        {result && (
          <div className="mt-8 bg-black/40 rounded-2xl p-5 text-left shadow-inner border border-emerald-700">
            ...
            <p className="text-gray-300 text-sm"><b>File:</b> {result.filename}</p>
            <p className="text-gray-300 text-sm mt-1">
              <b>Prediction:</b>{" "}
              <span className={result.prediction === "Malicious" ? "text-red-500 font-semibold" : "text-emerald-400 font-semibold"}>
                {result.prediction}
              </span>
            </p>
            <p className="text-gray-300 text-sm mt-1">
